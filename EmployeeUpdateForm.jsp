...
<script type="text/javascript">
	...
	$(document).ready(function()
	{
		...
		// jquery-ui 캘린더(datapicker)를 불러오는 함수 처리
		$("#birthday").datepicker(
		{
		...
		$("#submitBtn").click(function()
		{
			...
			*/	  
			...
			{
				$("#err").html("최소 기본급보다 큰 기본급을 입력해주세요.");
				$("#err").css("display", "inline");
				return;												// -- submit 액션 처리 중단
			}
	...
	function ajaxRequest()
	{
		...
		// 『$.post()』 / 『$.get()』
		// -- jQuery에서 AJAX를 사용해야 할 경우 지원해 주는 함수
	...
	{	
		$.post("ajax2.action", {employeeId :$("#employeeId").val(), ssn2 : $("#ssn2").val()}, function(data)
		{
			if (data == 0)	// 특정 employeeId의 주민번호 뒷자리가 틀릴경우 count 결과 0
			{
				...
				$("#err").css("display", "inline");
				$("#ssn2").val('');
				$("#ssn2").css("background-color", "#ccc");
				$("#ssn2").focus();
				return;
			}
			else if (data == 1)	// 특정 employeeId의 주민번호 뒷자리가 일치할경우 count 결과 1
			{
				$("#employeeForm").submit();	
			}
		});
